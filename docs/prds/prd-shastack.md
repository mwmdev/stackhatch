...
- Environment variables via `.env.local` (gitignored): `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL` (default: `claude-sonnet-4-20250514`), `DATABASE_URL` (default: `file:./data/shastack.db`)
...
type ServicesSubtype = 'auth' | 'payments' | 'notifications' | 'search' | 'file-processing' | 'custom'
...
type NodeSubtype = ClientSubtype | ApiSubtype | ServicesSubtype | DataSubtype | InfraSubtype | ExternalSubtype
...
**Technical Details:**
- Create API routes:
  - `GET /api/settings` — returns all settings as a key-value object `{ apiKey: "...", model: "...", theme: "..." }`
  - `PATCH /api/settings` — accepts a partial key-value object and upserts each key. Valid keys: `apiKey` (string), `model` (enum of Claude model IDs), `theme` ('light' | 'dark' | 'system')
- The `apiKey` value should be stored as-is (no encryption for v1 local-only use, but add a TODO comment noting this should be encrypted for VPS deployment)
- Default model value if unset: `claude-sonnet-4-20250514`
- The settings API reads from env vars as fallback: if `ANTHROPIC_API_KEY` env var is set and no `apiKey` in DB, use the env var. DB settings override env vars.
- Write integration tests

**Acceptance Criteria:**
- [ ] Settings persist across app restarts
- [ ] Environment variables serve as fallback for unset DB settings
- [ ] DB settings override env vars when both exist
...
- Create API route `POST /api/projects/[id]/chat` that:
  ...
  4. Loads settings to get the API key and model
  5. If no API key is configured, return 400 with error "API key not configured"
...
- Write a comprehensive Playwright E2E test that:
  ...
  5. Eventually triggers architecture generation (this may require a mock or a real API call with a test key — configure via env var `E2E_ANTHROPIC_API_KEY`)
...
- This test can be configured to run against a mock AI backend (for CI) or real Anthropic API (for local validation). Create a test helper that intercepts the `/api/projects/[id]/chat` route and returns canned SSE responses when `E2E_ANTHROPIC_API_KEY` is not set.
